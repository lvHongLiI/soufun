...
var table = $('#data-table').DataTable({
    ...
    columnDefs: [{ // 定义表格样式
        ...
        render: function (data, type, row, meta) {
            ...
            return '<td><img onClick="house_edit(\'查看\', \'/admin/house/show?id=' + row.id + '\')" title="查看"' +
                ' class="picture-thumb" src="http://localhost:8131/upload/'+data+'" width="60 px" height="50 px"></td>';
        }
    }, {
        targets: 7,
        render: function (data, type, row, meta) {
    ...
    ajax: {
        type: "POST",
        url: "/admin/house/findAll", // 服务器url
        cache: false,
        data: function (data) {
            var postData = {},
            ...
            if (createTimeMax!=''){
                postData.createTimeMax =  new Date(createTimeMax);
            }
            if (createTimeMin!=''){
                postData.createTimeMin = new Date(createTimeMin);
            }
